...
def _build_legend(all_tags, tag_color, letter_map, last_polled_at=""):
    ...
    help_panel = (
        ...
        "<b>Last marker</b> — white letter instead of grey = most recent fix for that tag, "
        "computed on all data before the filters: if that fix is excluded by the status or "
        "time filter, no white letter is shown for that tag<br>"
        "</div>"
        "<div style='margin-top:8px'>"
